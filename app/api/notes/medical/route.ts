...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only professionals can access medical notes
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only clinicians can create medical notes
    if (userRole !== Role.CLINICIAN && userRole !== Role.ADMIN) {
      return NextResponse.json(
        { error: "Only clinicians can create medical notes" },
        { status: 403 }
      );
    }

    const body = await request.json();
    const validation = medicalNoteSchema.safeParse(body);
    ...
    const { childId, noteType, subjective, objective, assessment, plan, medications, vitals } =
      validation.data;
